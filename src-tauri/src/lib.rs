...
fn resolve_bin(app: &tauri::AppHandle, name: &str) -> Result<String, String> {
    ...
    let path = resource_dir.join("bin").join(format!(
        "{}-{}{}",
        name,
...
fn kill_leftover_sidecars(app: &tauri::AppHandle) {
    ...
    for line in contents.lines() {
        let Ok(pid) = line.trim().parse::<usize>() else {
            continue;
        };
        ...
        let belongs_to_us = process
            .exe()
            .is_some_and(|exe| exe.starts_with(&resource_dir));
        if belongs_to_us {
            log::info!(
                "killing leftover sidecar process from a previous run: {}",
                pid
            );
            process.kill();
        } else {
            log::warn!(
                "PID {} in stale sidecar pidfile no longer matches our binaries, skipping",
                pid
            );
        }
    }
...
fn write_sidecar_pids(app: &tauri::AppHandle) {
    ...
    let Some(sidecars) = app.try_state::<Sidecars>() else {
        return;
    };
    let pids: String = sidecars
        .0
        .lock()
        .unwrap()
        .iter()
        ...
        .setup(|app| {
            ...
            let (mut nats_rx, nats_child) = app
                ...
                .unwrap_or_else(|e| {
                    fatal(
                        app.handle(),
                        &format!("Failed to start the bundled NATS server: {}", e),
                    )
                });
            ...
            let resolve =
                |name| resolve_bin(app.handle(), name).unwrap_or_else(|e| fatal(app.handle(), &e));
            ...
            for worker_id in 0..num_workers {
                let (mut worker_rx, worker_child) = app
                    ...
                    .unwrap_or_else(|e| {
                        fatal(
                            app.handle(),
                            &format!("Failed to start worker process {}: {}", worker_id, e),
                        )
                    });
                tauri::async_runtime::spawn(async move {
