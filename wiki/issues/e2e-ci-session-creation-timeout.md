...
**Type**: issue
**Summary**: The e2e suite passed 100% of the time run locally but failed every single spec in GitHub Actions on both matrix legs with `WebDriverError: ... timeout ... POST /session`. Linux is resolved: `tauri-driver` started before any display existed. Windows is still open: the initial "mismatched `msedgedriver`" diagnosis was disproven by its own fix's diagnostics (driver and WebView2 Runtime versions matched exactly); real cause unknown, app-log-file capture added to find it on the next run.
**Tags**: #issue #open #e2e #testing #ci #xvfb #webview2
