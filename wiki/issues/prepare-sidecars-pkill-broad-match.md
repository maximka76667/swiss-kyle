...
**Type**: issue
**Summary**: Resolved. `scripts/prepare-sidecars.ts` used `pkill -f worker` to clean up stale sidecar processes before rebuilding, which matched the substring "worker" in *any* process's full command line — including VS Code's own renderer processes on Linux — closing the editor window on every `bun tauri dev` run.
