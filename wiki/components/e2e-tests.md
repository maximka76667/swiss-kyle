...
**Sources**: [[e2e/wdio.conf.ts]], [[e2e/package.json]], [[e2e/repeat.ts]], [[e2e/specs/smoke.spec.ts]], [[e2e/specs/sidecars.spec.ts]], [[e2e/specs/edit-video.spec.ts]], [[e2e/specs/doc-converter.spec.ts]], [[e2e/specs/merge-pdfs.spec.ts]], [[e2e/specs/navigation.spec.ts]], [[e2e/support/selectors.ts]], [[e2e/support/drag-drop.ts]], [[e2e/support/navigate.ts]], [[e2e/support/click.ts]], [[e2e/support/ffmpeg.ts]], [[src-tauri/capabilities/default.json]], [[crates/shared/src/lib.rs]], [[ui/src/hooks/use-file-drop.ts]], [[.env.development]], [[.github/workflows/ci.yml]]
...
`ui/src/main.tsx` uses `MemoryRouter` with `initialEntries={['/edit-video']}` (→ [[wiki/components/frontend]]), so every fresh launch boots directly onto the Edit Video tool — no navigation needed to reach it. Reaching Doc Converter or Merge PDFs requires driving the sidebar (see `openTool` below).
...
- **`navigate.ts` — `openTool(label)`**: clicks the sidebar's `[data-slot="sidebar-trigger"]` toggle to expand it, then clicks the tool's label text via `byText`. Needed for any tool other than Edit Video (see App boot state above).
...
- **`ffmpeg.ts` — `countDecodedVideoFrames(path)`**: some correctness bugs can't be told apart from success by watching the UI alone — e.g. a video with zero actual video frames (audio-only) still makes ffmpeg exit 0 and the job report `Done`. This runs the bundled `ffmpeg` sidecar (found by scanning `src-tauri/binaries/` for a `ffmpeg-*` entry, not a hardcoded target triple, so it resolves on any platform) to fully decode a file's first video stream to `/dev/null`-equivalent and parses the final `frame=` count from its stderr stats output. Used by `edit-video.spec.ts`'s phone-clip regression test to verify the output actually contains video, not just that the job succeeded.
- **`ffmpeg.ts` — `getVideoDimensions(path)`**: reads a video's pixel dimensions by parsing `ffmpeg -i <path>`'s own stderr banner — no ffprobe sidecar is bundled, only `ffmpeg-*`. Used by the crop regression test to confirm the output is actually smaller than the source. The regex must anchor on `,\s*(\d+)x(\d+)(?=[\s,])`, not a bare `(\d+)x(\d+)`: the stream line also has a hex codec tag like `0x31637661` before the real resolution, and a looser match picks that up first (`width=0`) — a real bug that made every crop assertion fail with a nonsensical `< 0` threshold until traced to the regex, not the crop feature itself.
...
| `edit-video.spec.ts`    | Accepts a dropped video (`fixtures/sample.mp4`); rejects an unsupported extension (`fixtures/unsupported.txt`) with a toast; submits a real cut job and waits for "Done" in job history (happy path — real ffmpeg run); cuts `fixtures/phone-sparse-keyframes.mp4` (the real clip that surfaced the keyframe-spacing bug, → [[wiki/components/worker]] edit_video.rs) and verifies via `countDecodedVideoFrames` that the output actually contains video frames; submits without touching any control (regression test — a slow/stuck metadata load once left "Submit job" looking permanently disabled); drops several videos in a row without waiting for earlier jobs, asserting the controls stay usable after each (regression test — the `loadId`/remount bug, → [[wiki/components/frontend]]); drags a crop handle and asserts the on-screen indicator reflects a smaller selection; submits a crop job and verifies via `getVideoDimensions` that the output is actually smaller than the source                                                                                    |
...
| `navigation.spec.ts`    | Drives `openTool()` for every tool (Doc Converter, Merge PDFs, Diagnostics, Edit Video) and asserts each one's page-specific marker renders — decoupled from each tool's own drop/validation test, added to catch navigation regressions directly (→ [[wiki/issues/e2e-linux-native-click-unreliable]]) |

### Dragging the crop handle (`edit-video.spec.ts`'s `dragCropHandle`)

Two unrelated WebDriver/Actions-API issues had to be worked around, both discovered empirically:

- `.action('pointer').move({ origin: 'pointer', x: dx, y: dy })` (relative offset from the pointer's current position) is unreliable — [webdriverio/webdriverio#4134](https://github.com/webdriverio/webdriverio/issues/4134), never fixed upstream. The fix is absolute viewport coordinates (`origin: 'viewport'`, computed from `element.getLocation()`/`getSize()`) for every move step, not a delta from wherever the pointer already is.
- The crop handle can exist in the DOM (so `waitForExist`/`waitForDisplayed` pass) before `crop-overlay.tsx`'s own `ResizeObserver`-driven layout measurement has run — grabbing its position at that moment reads the pre-layout `scale: 0` fallback, collapsed to one point. `dragCropHandle` polls `getSize()` until it reports a real, non-zero size before computing where to drag to.

Both looked, for a long time, like the crop feature itself was broken — it wasn't. The actual regression (`getVideoDimensions`' hex-codec-tag regex bug, see `ffmpeg.ts` above) turned out to be in the test helper, not the drag mechanism or the feature; worth remembering that a WebDriver-flavored failure signature doesn't guarantee a WebDriver-flavored cause.
...
- `sample.mp4` (~15KB, 320x240) — plain valid video for the edit-video happy path and the crop regression test.
- `unsupported.txt` — deliberately wrong extension for all three tools' validation tests.
- `phone-sparse-keyframes.mp4` — a real phone-recorded clip with wide keyframe spacing; this is the actual file that originally surfaced the `edit_video.rs` stream-copy bug (→ [[wiki/components/worker]]). Used only by the frame-count regression test, not the plain happy-path test.
...
- Open (Windows), resolved (Linux): CI (`.github/workflows/ci.yml`, added after this component's local-only testing history above) failed every spec on both matrix legs with a `tauri-driver` session-creation timeout — Linux fixed by wrapping the step in `xvfb-run` (tauri-driver is spawned in `onPrepare`, outside @wdio/xvfb's per-worker wrapping); Windows's fix (installing a WebView2-Runtime-matched `msedgedriver` instead of the mismatched one GH preinstalls) didn't take on the first attempt, root-caused to pwsh silently swallowing a failed native command, now hardened but unverified (→ [[wiki/issues/e2e-ci-session-creation-timeout]]).
